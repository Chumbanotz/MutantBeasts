...
public class CreeperMinionTrackerScreen extends Screen {
	...
	@Override
	public void tick() {
		if (this.creeperMinion == null || !this.creeperMinion.isAlive() || this.creeperMinion.isAggressive()) {
			this.minecraft.player.closeScreen();
		}
	}

	private String alwaysShowName() {
		return "Always Show Name: " + I18n.format(this.alwaysShowName ? "options.on" : "options.off");
	}

	private String canDestroyBlocks() {
		return "Destroys Blocks: " + I18n.format(this.canDestroyBlocks ? "options.on" : "options.off");
	}

	private String canRideOnShoulder() {
		return "Can Ride On Shoulder: " + I18n.format(this.canRideOnShoulder ? "options.on" : "options.off");
	}
	...
	public void render(int screenX, int screenY, float elapsedPartialTicks) {
		...
		this.font.drawString("Health", this.guiX + 13, this.guiY + 28, 4210752);
		this.font.drawString("Explosion", this.guiX + 13, this.guiY + 48, 4210752);
		this.font.drawString("Blast Radius", this.guiX + 13, this.guiY + 68, 4210752);
		...
		this.drawCenteredString(this.font, this.creeperMinion.canExplodeContinuously() ? "Continuous" : "One-time", this.guiX + this.xSize / 2 + 38, this.guiY + 50, 16777215);
