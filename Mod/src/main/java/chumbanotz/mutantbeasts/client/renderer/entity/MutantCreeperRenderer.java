...
@OnlyIn(Dist.CLIENT)
public class MutantCreeperRenderer extends MobAutoTextureRenderer<MutantCreeperEntity, MutantCreeperModel> {
	...
	protected void preRenderCallback(MutantCreeperEntity entitylivingbaseIn, float partialTickTime) {
		...
		if (entitylivingbaseIn.deathTime > 0) {
			float f = (float)entitylivingbaseIn.deathTime / MutantCreeperEntity.MAX_DEATH_TIME;
			scale -= f * 0.4F;
		}
